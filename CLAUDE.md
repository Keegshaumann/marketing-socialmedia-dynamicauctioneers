...
- **Live golden extraction ✅ (2026-07-17):** the DP3060 golden run passed end to end on free credits. Extraction is sectioned (D21), non-strict tool use (D23: strict grammar rejects even a flat section as "too complex"), with opt-in text mode + pacing (D22) to run under the entry-tier 10k-input-tokens/min cap. Both sentinels caught unprompted (garage conflict + flatlet), POPIA verified on real PII, money facts match the golden exactly; residual diffs are formatting/normalization. Native PDF + strict-off tool use stay the defaults.
- **Next:** small code-side normalizers on extracted values (dates->ISO, `title_type`->enum, institution/zoning canonical case) so records match the golden's canonical form; then the external-credential open questions (§10).
