...
| **5** | Distribution v1: GHL Social Planner posting + per-channel status (D11), alert mailer generation, manual packs; Prop Data feed if access confirmed; generated buyer-facing Property Report | a price drop goes from record edit to live channels in minutes | 🟡 routing + manual packs + status log + GHL scaffold done; live posting needs the GHL token (Q10/Q13) |
